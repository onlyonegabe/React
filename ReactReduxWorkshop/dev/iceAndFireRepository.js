...
const get = url => axios.get(url, API_REQUEST_HEADER);

const getPage = (category, munge) => (page, pageSize) =>
  get(`${API_URL}/${category}?page=${page}&pageSize=${pageSize}`).then(
    response => response.data.map(munge)
  );

const mungeCharacter = character => ({
